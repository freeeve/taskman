...
func cmdBlocked(args []string) error {
	...
	if *unblock {
		...
		path, ok, err := store.UnblockLane(projDir, lane, note, date)
		if err != nil {
			return err
		}
		if !ok {
	...
	if fs.NArg() == 0 {
		return printBlocked(store.ReadBlocked(projDir))
	}
	if fs.NArg() < 2 {
	...
	path, err := store.SetBlock(projDir, lane, message, date)
	if err != nil {
		return err
	}
...
// printBlocked renders the stall board, unblocked entries flagged so the
// raising session sees the response and can clear its own block.
func printBlocked(blocks []store.Block) error {
	if len(blocks) == 0 {
